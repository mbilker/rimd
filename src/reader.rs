...
use crate::util::{fill_buf, latin1_decode, read_byte};
use crate::{
    Event, MetaCommand, MetaEvent, MidiMessage, SMFError, SMFFormat, Track, TrackEvent, SMF,
};
...
impl SMFReader {
    ...
    fn next_event(
        ...
        match stat {
            ...
            _ => {
                let msg = if (stat & 0x80) == 0 {
                    // this is a running status, so assume we have the same status as last time
                    MidiMessage::next_message_running_status(last_stat, stat, reader)?
                } else {
    ...
    fn parse_track(reader: &mut dyn Read) -> Result<Track, SMFError> {
        ...
        loop {
            let last = {
                // use status from last midi event, skip meta events
                let mut last = 0u8;
                    ...
                    match event.event {
                        Event::Meta(ref me) => match me.command {
                            MetaCommand::CopyrightNotice => {
                                copyright = Some(latin1_decode(&me.data))
                            }
                            MetaCommand::SequenceOrTrackName => {
                                name = Some(latin1_decode(&me.data))
                            }
                            _ => {}
                        },
                        _ => {}
