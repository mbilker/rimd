...
pub enum Status {
    ...
    SongSelect = 0xF3,
    TuneRequest = 0xF6, // F4 and 5 are reserved and unused
    SysExEnd = 0xF7,
    TimingClock = 0xF8,
    Start = 0xFA,
    Continue = 0xFB,
...
impl MidiMessage {
    ...
    pub fn next_message_given_status(
        stat: u8,
        reader: &mut dyn Read,
        ...
        match MidiMessage::data_bytes(stat) {
            ...
            -1 => {
                return Err(MidiError::OtherErr("Don't handle variable sized yet"));
            }
            -2 => {
                //println!("skipping sysex");
                // skip SysEx message
                while {
            ...
            _ => {
                return Err(MidiError::InvalidStatus(stat));
            }
        }
    ...
    pub fn next_message_running_status(
        stat: u8,
        databyte: u8,
        reader: &mut dyn Read,
    ) -> Result<MidiMessage, MidiError> {
        ...
        match MidiMessage::data_bytes(stat) {
            0 => {
                panic!("Can't have zero length message with running status");
            }
            1 => {} // already read it
            2 => {
                ret.push(read_byte(reader)?);
            ...
            -2 => {
                return Err(MidiError::OtherErr(
                    "Running status not permitted with meta and sysex event",
                ));
            }
            _ => {
                return Err(MidiError::InvalidStatus(stat));
    ...
    pub fn polyphonic_aftertouch(note: u8, pressure: u8, channel: u8) -> MidiMessage {
        MidiMessage {
            data: vec![
                make_status(Status::PolyphonicAftertouch, channel),
                note,
                pressure,
            ],
        }
